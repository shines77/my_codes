...
int random_pick_number_and_throw()
{
    //
}
...
{
    group_info * group_list = (group_info *)malloc(sizeof(group_info) * groups);
    if (group_list == NULL)
        return -1;
    ...
    do {
        ...
        while (pick_nums != length || !is_final_result(group_list, length, groups, averages, remain)) {
            ...
            random_pick_number_and_throw(group_list, length);
        }
