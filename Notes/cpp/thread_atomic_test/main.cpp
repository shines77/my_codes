...
void thread_test_proc(const Task & task, uint32_t hardware_concurrency)
{
    if (!task.isCanceled()) {
        std::cout << ">> [" << task.index << "]" << "send control command." << std::endl;

        //uint32_t tls_counter = g_counter.fetch_add(1) + 1;
        uint32_t tls_counter = ++g_counter;
...
void thread_atomic_test(uint32_t thread_num)
{
    ...
    std::cout << "--------------------- thread_atomic_test() -----------------------" << std::endl << std::endl;
...
#if 1
            tasks[i].index = i + 1;
            threads[i].reset(new std::thread(thread_test_proc, tasks[i], thread_num));
#else
            tasks.get()[i].index = i + 1;
            threads.get()[i].reset(new std::thread(thread_test_proc, tasks.get()[i], thread_num));
#endif
        }
...
void thread_atomic_test2(uint32_t thread_num)
{
#if 1
    ...
    std::cout << "--------------------- thread_atomic_test2() ----------------------" << std::endl << std::endl;
...
{
    thread_atomic_test(max_thread_cnt);
    g_counter.store(0);
    thread_atomic_test2(max_thread_cnt);
